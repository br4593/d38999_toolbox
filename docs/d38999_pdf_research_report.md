...
- Secondary-source exact PN scrape plus overlap classification: `data/d38999_federalconnectors_secondary_source.json`
- Unified valid D38999 PN database across all PN-bearing JSON sources: `data/d38999_valid_part_numbers.json`
- Visual inventory and safe SVG recreation notes: `data/d38999_visual_assets.json`

## Secondary-Source Harvest

- `data/d38999_federalconnectors_secondary_source.json` is generated from the hierarchical part-number index at `https://d38999.federalconnectors.com/D38999` using `scripts/scrape_federalconnectors.py`.
- This file is intentionally separate from `data/d38999_verified_part_numbers.json`. The federalconnectors site is useful for harvesting exact orderable strings, but it is treated as a secondary source rather than as manufacturer-catalog proof.
- The current bounded crawl targets manufacturer-relevant Series III military prefixes `20WB`, `20WC`, `24WC`, and `26FD`.
- Each scraped exact part number is cross-checked against:
	- the exact manufacturer-verified dataset (`exactVerifiedOverlaps`)
	- the manufacturer-backed slash-sheet support dataset (`importableOverlaps`)
- Current scrape summary: 631 exact site part numbers, 3 exact overlaps with the verified set (`D38999/20WB5SN`, `D38999/20WC35PN`, `D38999/26FD35PN`), and 627 exact site part numbers whose combinations are already supported by local manufacturer-backed rules.

## Unified Valid PN Database

- `data/d38999_valid_part_numbers.json` is generated by `scripts/build_valid_d38999_pns.py`.
- It merges all current PN-bearing JSON sources in the repo:
	- `data/d38999_verified_part_numbers.json`
	- `data/d38999_part_number_examples.json`
	- `data/d38999_federalconnectors_secondary_source.json`
	- `data/qpl_1122_part_numbers.json`
- The file keeps only exact `D38999/...` part numbers and normalizes them into one record per part number with decoded fields, source-presence flags, and source evidence.
- Current unified summary: 7,572 unique valid D38999 part numbers, including 5,835 QPL-listed parts, 1,984 exact federalconnectors matches, and 3 manufacturer-verified exact catalog matches.
