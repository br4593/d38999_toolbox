...
|   |-- d38999_valid_part_numbers/      # SHARDED dataset: index.json + part_numbers_NNN.json
|   |-- d38999_environment_classification/  # SHARDED: index.json + connector_records_NNN.json
...
- `data/d38999_valid_part_numbers/` — `index.json` (metadata) plus
  `part_numbers_NNN.json` chunks.
- `data/d38999_environment_classification/` — `index.json` plus
  `connector_records_NNN.json` chunks.

Each shard stays under 50 MiB. Code reads either layout transparently via
`scripts/dataset_io.py` (`load_dataset(path)` accepts the legacy `.json` path),
and `scripts/build_valid_d38999_pns.py` regenerates the shards. The
